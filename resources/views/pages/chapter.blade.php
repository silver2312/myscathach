...
<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-12" style="text-align: center">
            ...
            <a href="{{url('xemtruyen/'.$ch->truyenC->bk_slug)}}" style="text-decoration: none;"><h3>{{$ch->truyenC->bk_name}}</h3></a>
            ...
            <nav aria-label="Page navigation example" style="text-alignt: center;">
                <ul class="pagination">
                    ...
                    <li>                        
                        <select class="page-link select-chapter" name="select-chapter" style="position: relative;width:250px;height:30px;">
                            @foreach($all_ch as $key => $vle)
                                <option width="150px;" value="{{url('doctruyen/'.$vle->truyen_id.'/'.$vle->position)}}">Chương {{$vle->position}}: {{$vle->header}}</option>
                            @endforeach
                        </select>
                    </li>
            ...
            <nav aria-label="Page navigation example" style="text-alignt: center;">
                <ul class="pagination">
                    ...
                    <li> 
                        <select class="page-link select-chapter" name="select-chapter" style="position: relative;width:250px;height:30px;">
                            @foreach($all_ch as $key => $vle)
                                <option value="{{url('doctruyen/'.$vle->truyen_id.'/'.$vle->position)}}">Chương {{$vle->position}}: {{$vle->header}}</option>
                            @endforeach
                        </select>     
                    </li>
                    <li><a class="page-link {{$ch->position==$max_id->position ? 'isDisable' : ''}}" href="{{url('doctruyen/'.$ch->truyen_id.'/'.$next_ch)}}">Sau</a></li>
